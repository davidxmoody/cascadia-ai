...
class StateFeatures:
    ...
    @cached
    def _habitat_areas(
        self, habitat: Habitat, placed_tile: tuple[HexPosition, int, int] | None
    ):
        if placed_tile is None:
            return HabitatAreas(habitat, self.state.env.tiles)

        ha = deepcopy(self._habitat_areas(habitat, None))
        pos, index, rot = placed_tile
        tile = self.state.tile_display[index].rotate(rot)
    ...
    def _hreward(
        ...
        if placed_tile is not None:
            changed_habitats = self.state.tile_display[placed_tile[1]].habitats
            if habitat in changed_habitats:
                ha = self._habitat_areas(habitat, placed_tile)
                return ha.get_best_reward(pos)

        ha = self._habitat_areas(habitat, None)
        return ha.get_best_reward(pos)
    ...
    def get_features(
        ...
        for w in Wildlife:
            data[F[f"{w.value}_best_adjacent_wreward"]] = self._best_adjacent_wreward(
                w, placed_wildlife, None if action is None else action.tile_position
            )

            data[F[f"{w.value}_best_unoccupied_wreward"]] = (
                self._best_unoccupied_wreward(
                    w,
                    placed_wildlife,
                    (
                        None
                        if action is None
                        else (action.tile_position, action.tile_index)
                    ),
                )
            )
